...
-- |
bellmanFord
    :: (Ord v, Hashable v, Show v, Show (DG.IdxEdge v meta), Eq (DG.IdxEdge v meta))
...
-- |
relax
    :: (Show v, Ord v, Hashable v, Show (DG.IdxEdge v meta)) => DG.VertexId
    -> BF s v meta ()
relax vertex = do
    graph      <- R.asks sGraph
...
findNegativeCycle
    :: (Ord v, Show v, Show (DG.IdxEdge v meta), Hashable v)
    => BF s v meta ()
...
check
    :: (DG.HasWeight meta Double, Eq (DG.IdxEdge v meta), Show (DG.IdxEdge v meta))
    => DG.VertexId
    -> BF s v meta Bool
check source = do
    graph      <- R.asks sGraph
